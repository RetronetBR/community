...
Listar referências técnicas e comunitárias úteis para o RetroNet SDK.
...
- Uso no RetroNet: possível base para `rn-ipx`, gateway IPX e compatibilidade com redes DOS e jogos antigos
...
- Uso no RetroNet: referência prática para integração e testes de rede IPX
...
- Uso no RetroNet: base para `RNTSR`, polling, popup de notificações e integração com `INT 2Fh`

### 4. ChillRuns
- Link: https://youtube.com/@chillruns6221
- Tema: programação MS-DOS
- Descrição: conteúdo sobre DOS e desenvolvimento retro
- Uso no RetroNet: referência para desenvolvimento de clientes DOS, memória, interrupções e aplicações retro
...
- Uso no RetroNet: referência para programação próxima ao hardware e preservação retro
...
- Uso no RetroNet: referência complementar para hardware retro e ecossistema técnico
...
- Uso no RetroNet: referência para futuras interfaces gráficas, `librnui` e manipulação VGA
...
- Uso no RetroNet: referência para engenharia do SDK e boas práticas em C/C++

### 9. Pascal/MS-DOS
- Link: https://youtu.be/h8OdxRemIEo
- Tema: Pascal e programação MS-DOS
- Descrição: referência para Pascal em ambiente DOS
- Uso no RetroNet: referência para bindings Pascal, exemplos em Turbo Pascal/Borland Pascal e possível uso com Turbo Vision
...
- Uso no RetroNet: plataforma de testes, compatibilidade DOS e referência de preservação ativa
...
- Uso no RetroNet: reduzir memória, melhorar desempenho, criar TSRs menores e clientes eficientes
...
- Uso no RetroNet: builds reproduzíveis, CI/CD e toolchain moderna para clientes DOS
...
- Uso no RetroNet: base para interfaces gráficas DOS, `librnui` e testes em emulador
...
- Uso no RetroNet: popups, widgets, interfaces gráficas simples e manipulação direta de vídeo
...
- Uso no RetroNet: alertas sonoros, `librnaudio`, notificações do `RNTSR` e áudio retro

### 16. Sound Blaster 16
- Link: não informado
- Tema: Sound Blaster 16
- Descrição: ainda falta referência consolidada
- Uso no RetroNet: futura trilha para áudio PCM, WAV, DMA, IRQ, mixer e alertas customizados

## Status

As referências são pontos de partida para pesquisa, implementação e testes.

## Próximos passos

- organizar uma lista curta de referências prioritárias
- separar o que serve para pesquisa do que serve para implementação
